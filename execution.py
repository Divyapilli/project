...
# ================== PDF FONT AUTO-DETECTION ==================
NOTO = "/usr/share/fonts/truetype/noto/"

SCRIPT_FONT_RANGES = [
    (0x0C00,0x0C7F,"NotoSansTelugu-Regular.ttf"),
    (0x0900,0x097F,"NotoSansDevanagari-Regular.ttf"),
    (0x0980,0x09FF,"NotoSansBengali-Regular.ttf"),
    (0x0B80,0x0BFF,"NotoSansTamil-Regular.ttf"),
    (0x0C80,0x0CFF,"NotoSansKannada-Regular.ttf"),
    (0x0D00,0x0D7F,"NotoSansMalayalam-Regular.ttf"),
    (0x0A80,0x0AFF,"NotoSansGujarati-Regular.ttf"),
    (0x0A00,0x0A7F,"NotoSansGurmukhi-Regular.ttf"),
    (0x0B00,0x0B7F,"NotoSansOriya-Regular.ttf"),
    (0x0D80,0x0DFF,"NotoSansSinhala-Regular.ttf"),
    (0x0600,0x06FF,"NotoSansArabic-Regular.ttf"),
    (0x0750,0x077F,"NotoSansArabic-Regular.ttf"),
    (0xFB50,0xFDFF,"NotoSansArabic-Regular.ttf"),
    (0x0590,0x05FF,"NotoSansHebrew-Regular.ttf"),
    (0xFB00,0xFB4F,"NotoSansHebrew-Regular.ttf"),
    ...
    (0x0530,0x058F,"NotoSansArmenian-Regular.ttf"),
    (0x10A0,0x10FF,"NotoSansGeorgian-Regular.ttf"),
    (0x1200,0x137F,"NotoSansEthiopic-Regular.ttf"),
    (0x0400,0x04FF,"NotoSans-Regular.ttf"),
    (0x0370,0x03FF,"NotoSans-Regular.ttf"),
    (0x4E00,0x9FFF,"NotoSans-Regular.ttf"),
    (0x3040,0x30FF,"NotoSans-Regular.ttf"),
    (0xAC00,0xD7AF,"NotoSans-Regular.ttf"),
]

def detect_pdf_font(text):
    for ch in text[:500]:
        cp = ord(ch)
        if cp < 0x0080:
            continue
        for start, end, fname in SCRIPT_FONT_RANGES:
            if start <= cp <= end:
                fpath = NOTO + fname
                return fpath if os.path.exists(fpath) else NOTO + "NotoSans-Regular.ttf"
    return NOTO + "NotoSans-Regular.ttf"
...
def _safe_filename(name, fmt):
    name = (name or "transformed_output").strip()
    safe = "".join(c for c in name if c.isalnum() or c in " _-").strip() or "transformed_output"
...
def build_download_file(text, fmt, filename):
    ...
    try:
        if fmt == "TXT":
            ...
            pdf.cell(ew, 7, f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}",
            ...
            for kind, content in blocks:
                if kind == "heading":
                    ...
                    pdf.multi_cell(ew, 9, content)
                    pdf.ln(2)
                else:
                    pdf.set_font("Noto", "", 11)
                    pdf.set_text_color(30, 30, 30)
                    pdf.multi_cell(ew, 7, content)
                    pdf.ln(4)
            pdf.output(filepath)
        ...
        return None, f"⚠️ Error creating {fmt} file: {e}"
